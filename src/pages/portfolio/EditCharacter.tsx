...
import { useCharacterProfile, useUpdateCharacterProfile } from '@/hooks/useCharacters';
...
const EditCharacter: React.FC = () => {
  ...
  const { data: character } = useCharacterProfile(id || '');
  const { mutate: updateProfile, isPending } = useUpdateCharacterProfile();
  ...
  const [abilities, setAbilities] = useState([
    { ability_type: 'power', name: '', description: '' }
  ]);
  ...
  const [inspirations, setInspirations] = useState([
    { influence_name: '', influence_type: '', why_they_matter: '' }
  ]);

  useEffect(() => {
    if (character) {
      setForm({
        story_id: character.story_id || 'none',
        full_name: character.full_name || '',
        title: character.title || '',
        aliases_nicknames: character.aliases_nicknames || '',
        species_race: character.species_race || '',
        gender: character.gender || '',
        age: character.age || '',
        height: character.height || '',
        weight: character.weight || '',
        summary_tagline: character.summary_tagline || '',
        core_motivation: character.core_motivation || '',
        backstory: character.backstory || '',
        role_in_story: character.role_in_story || '',
        notable_traits: (character.notable_traits_array || []).join(', '),
        affiliations: (character.affiliations_array || []).join(', ')
      });
      setImageUrl(character.img_url || '');
    }
  }, [character]);
  ...
  const handleFileUpload = (results: Array<{ success: boolean; url?: string }>) => {
    ...
    updateProfile(
      {
        ...
        notable_traits_array: form.notable_traits
          .split(',')
          .map(t => t.trim())
          .filter(Boolean),
        affiliations_array: form.affiliations
          .split(',')
          .map(t => t.trim())
          .filter(Boolean),
        ...
        onSuccess: async (data) => {
          if (abilities.length) {
            await supabase
              ...
              .insert(abilities.map(a => ({ ...a, character_id: data.id })));
          }
          if (relationships.length) {
            await supabase
              .from('character_relationships')
              .insert(
                relationships.map(r => ({ ...r, character_id: data.id }))
              );
          }
          if (inspirations.length) {
            await supabase
              ...
              .insert(
                inspirations.map(i => ({ ...i, character_id: data.id }))
              );
          }
      ...
      <div className="pt-24 pb-16 px-8">
        <div className="max-w-3xl mx-auto">
          <Card className="bg-card/60">
            <CardHeader>
              <CardTitle className="mono-font text-foreground">Edit Character Profile</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Input
                  name="aliases_nicknames"
                  value={form.aliases_nicknames}
                  onChange={handleChange}
                  placeholder="Aliases or Nicknames"
                  className="mono-font"
                />
                ...
                <Textarea
                  name="summary_tagline"
                  value={form.summary_tagline}
                  onChange={handleChange}
                ...
                <div className="space-y-2">
                  ...
                  {abilities.map((ability, idx) => (
                    <div key={idx} className="grid grid-cols-3 gap-2">
                      <select
                        ...
                        onChange={e =>
                          handleAbilityChange(idx, 'ability_type', e.target.value)
                        }
                        className="mono-font border rounded p-2 bg-stone-950"
                      >
                      ...
                      <Input
                        value={ability.name}
                        onChange={e =>
                          handleAbilityChange(idx, 'name', e.target.value)
                        }
                        placeholder="Name"
                        className="mono-font"
                      />
                      <Input
                        ...
                        onChange={e =>
                          handleAbilityChange(idx, 'description', e.target.value)
                        }
                  ...
                  <Button type="button" variant="outline" onClick={addAbility} className="mono-font">
                    Add Ability
                  </Button>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {relationships.map((rel, idx) => (
                    <div key={idx} className="grid grid-cols-5 gap-2">
                      <select
                        ...
                        onChange={e =>
                          handleRelationshipChange(idx, 'relationship_type', e.target.value)
                        }
                        className="mono-font border rounded p-2 bg-stone-950"
                      >
                      ...
                      <Input
                        ...
                        onChange={e =>
                          handleRelationshipChange(idx, 'character_name', e.target.value)
                        }
                      ...
                      <Input
                        ...
                        onChange={e =>
                          handleRelationshipChange(idx, 'description', e.target.value)
                        }
                        placeholder="Description"
                        className="mono-font"
                      />
                      <Input
                        value={rel.related_character_id}
                        onChange={e =>
                          handleRelationshipChange(idx, 'related_character_id', e.target.value)
                        }
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={rel.is_bidirectional}
                          onChange={e =>
                            handleRelationshipChange(idx, 'is_bidirectional', e.target.checked)
                          }
                  ...
                  <Button type="button" variant="outline" onClick={addRelationship} className="mono-font">
                    Add Relationship
                  </Button>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {inspirations.map((ins, idx) => (
                    <div key={idx} className="grid grid-cols-3 gap-2">
                      <Input
                        value={ins.influence_name}
                        onChange={e =>
                          handleInspirationChange(idx, 'influence_name', e.target.value)
                        }
                        placeholder="Name"
                        className="mono-font"
                      />
                      <Input
                        value={ins.influence_type}
                        onChange={e =>
                          handleInspirationChange(idx, 'influence_type', e.target.value)
                        }
                        placeholder="Type"
                        className="mono-font"
                      />
                      <Input
                        value={ins.why_they_matter}
                        onChange={e =>
                          handleInspirationChange(idx, 'why_they_matter', e.target.value)
                        }
                  ...
                  <Button type="button" variant="outline" onClick={addInspiration} className="mono-font">
                    Add Inspiration
                  </Button>
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mono-font block mb-2">Profile Image</label>
                  {!imageUrl ? (
                    <SecureFileUpload
                      accept="image/*"
                      ...
                      storageFolder="character-profiles"
